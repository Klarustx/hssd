...
[yN,xN]=size(y);
%% %%%%%%%%%%%%%%%%%% BM3D Bluuring and Noising the picture %%%%%%%%%%%%%%%%%
experiment_number  =   6  ;
...
ssim_init = ssim_index(y_hat_init*255,y*255);

fprintf('Initialization method: %s\n', initType);
fprintf('Initial estimate ISNR: %4.2f, PSNR: %4.2f, SSIM: %4.4f \n', psnr_init-psnr_z, psnr_init, ssim_init);
figure(199);imshow(y_hat_init,[]);title(sprintf('BM3D(Initial),PSNR: %4.2fdB,SSIM: %4.4f',psnr_init,ssim_init),'fontsize',13);
...
save IDDBM3Dbarbara512BM3Dscenario6.mat y_hat ;

psnr = PSNR(y,y_hat,1,0);
isnr = psnr-psnr_z;
ssim = ssim_index(y_hat*255,y*255);

figure(299); imshow(y_hat,[]);title(sprintf('BM3D(Final),PSNR: %4.2fdB,SSIM: %4.4f',psnr,ssim),'fontsize',13);
